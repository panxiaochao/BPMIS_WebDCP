...
public class PageHtmlUtils {
	...
	private static String Li_Class_Previous_Disabled = "<li class=\"previous disabled\"><a href=\"#\">上一页</a></li>";// pdisabled=disabled
	...
	private static String Li_Class_Next_Disabled = "<li class=\"next disabled\"><a href=\"#\">下一页 </a></li>";// ndisabled=disabled
	private static String Li_Sheng = "<li><a href=\"#\">......</a></li>";
	/*
	 * ===================== 默认样式2 样式1//圆形====================
	 */
	...
	private static String DefaultPagination(int currentPage, int pageSize,
		...
		if (totalPages <= 5) {
			...
			for (int i = 1; i < totalPages + 1; i++) {
				if (currentPage == i)
					page.append(Li_Active_Content).append(i)
							.append("</a></li>");
				else
					page.append(Li_Content).append(i).append(");\">").append(i)
							.append("</a></li>");
			}
			if (currentPage == totalPages) // ==总页数
				page.append(Li_Class_Next_Disabled);
			else
				page.append(Li_Class_Next_s).append(currentPage + 1).append(
						Li_Class_Next_e);
			...
			if (currentPage < 5) {
				...
				page.append(Li_Content).append(currentPage - 2).append(");\">")
						.append(currentPage - 2).append("</a></li>");
				page.append(Li_Content).append(currentPage - 1).append(");\">")
						.append(currentPage - 1).append("</a></li>");
	 ...
	 * @return String
	 */
	public static String getPageDcp(Page page) {
	...
	private static String getPageDcp(int currentPage, int pageSize,
			int totalPages, int totalRecords) {
		StringBuffer page = new StringBuffer();
		page.append(Div_Class_Pagination).append(Ul_Class_Pager);// 先添加头
		// totalPages <=5
		if (totalPages <= 5) {
			if (currentPage == 1) // ==第一页
				page.append(Li_Class_Previous_Disabled);
	...
	public static void main(String[] args) {
		for (int i = 1; i < 5; i++) {
			System.out.println(new PageHtmlUtils().getPageDcp(i, 10, 20, 10));
		}
